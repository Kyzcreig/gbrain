...
const priorGate = process.env.GBRAIN_ZEMBED_DAYTIME_ENABLED;

afterEach(() => {
  if (priorGate === undefined) delete process.env.GBRAIN_ZEMBED_DAYTIME_ENABLED;
  else process.env.GBRAIN_ZEMBED_DAYTIME_ENABLED = priorGate;
});
...
describe('P3 cloud-only daytime enqueue', () => {
  test('default OFF performs no database work', async () => {
    delete process.env.GBRAIN_ZEMBED_DAYTIME_ENABLED;
    let calls = 0;
  ...
  test('persists one current revision without calling a document provider', async () => {
    process.env.GBRAIN_ZEMBED_DAYTIME_ENABLED = 'true';
